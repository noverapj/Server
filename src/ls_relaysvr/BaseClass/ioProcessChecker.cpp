...
FunctionTimeChecker::~FunctionTimeChecker()
{
	if( m_bFrequency )
	{
		...
		{
			if( m_pFileName )
				LOG.PrintTimeAndLog( 0, "%s %d Line 0x%x : T[%.3f]", m_pFileName, m_iFileLine, m_dwPacketID, fCompleteTime / 1000000 );
		}
	}
}
//////////////////////////////////////////////////////////////////////////
...
{ 
	m_dwLogTime = g_Config()->GetLogTime();
}
...
void ioProcessChecker::WriteLOG()
{
	...
	LOG.PrintTimeAndLog( LOG_DEBUG_LEVEL, "%s", szLOG );
