...
void CQueryData::GetReturns(char* pBuffer, int &nSize, int iBufferSize)
{
	if(m_Buffer->GetLength() > 0)
	{
		...
		if( nSize > iBufferSize )
			return;

		memcpy_s( pBuffer, iBufferSize, m_Buffer->GetBuffer(nOffset), nSize );
	}
}
