...
public:
	CatchMode( Room *pCreator );
	virtual ~CatchMode();
};

inline CatchMode* ToCatchMode( Mode *pMode )
{
	if( !pMode || pMode->GetModeType() != MT_CATCH )
		return NULL;

	return static_cast< CatchMode* >( pMode );
}
