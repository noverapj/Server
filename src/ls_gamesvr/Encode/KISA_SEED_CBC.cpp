...
UINT* chartoint32_for_SEED_CBC( IN BYTE *in, IN int nLen )
{
	unsigned int *data;
	int len,i;
...
BYTE* int32tochar_for_SEED_CBC( IN UINT *in, IN int nLen )
{
	unsigned char *data;
	int i;
...
// Set up input values for Key Schedule	
	A = ((UINT *)pbszUserKey)[0];
	B = ((UINT *)pbszUserKey)[1];
	C = ((UINT *)pbszUserKey)[2];
	D = ((UINT *)pbszUserKey)[3];
...
int SEED_CBC_Process( OUT KISA_SEED_INFO *pInfo, IN UINT *in, IN int inLen, OUT UINT *out, OUT int *outLen )
{
...
int SEED_CBC_Close( OUT KISA_SEED_INFO *pInfo, IN UINT *out, IN int *outLen )
{
