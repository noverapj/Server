...
SP2Packet::SP2Packet(TCHAR *buffer,int size) : CPacket( buffer, size )
{
}
...
SP2Packet::SP2Packet( DWORD dwUserIndex, SP2Packet &rhs )
{
	...
	*this << dwUserIndex;
	SetDataAdd( (TCHAR*)rhs.GetData(), rhs.GetDataSize() );
}
...
const TCHAR* SP2Packet::GetData() const
{
...
const TCHAR* SP2Packet::GetBuffer() const
{
	return &m_pBuffer[0];
}
...
void SP2Packet::SetDataAdd( TCHAR *buffer, int size, bool bCurPosReSet )
{
