...
#include "./ThailandBillingGetServer.h"
...
ThailandBillingGetServer *ThailandBillingGetServer::sg_Instance = NULL;
ThailandBillingGetServer::ThailandBillingGetServer( SOCKET s, DWORD dwSendBufSize, DWORD dwRecvBufSize ) : CConnectNode( s, dwSendBufSize, dwRecvBufSize )
{
	InitData();
}

ThailandBillingGetServer::~ThailandBillingGetServer()
{	
}

ThailandBillingGetServer &ThailandBillingGetServer::GetInstance()
{
	if(sg_Instance == NULL)
	{
		...
		sg_Instance = new ThailandBillingGetServer( INVALID_SOCKET, iSendBufferSize, iRecvBufferSize );
	}
	return *sg_Instance;
}

void ThailandBillingGetServer::ReleaseInstance()
{		
	SAFEDELETE( sg_Instance );
}

bool ThailandBillingGetServer::ConnectTo( bool bStart )
{
	ioINILoader kLoader( "ls_config_billingsvr.ini" );
	kLoader.SetTitle( "NETWORK" );

	char szServerIP[MAX_PATH];
	kLoader.LoadString( "ThailandBillingGetServerIP", "", szServerIP, MAX_PATH );

	int iSSPort = kLoader.LoadInt( "ThailandBillingGetServerPORT", 9000 );
...
void ThailandBillingGetServer::InitData()
{
	m_dwCurrentTime = 0;
	SetActive(false);
}

void ThailandBillingGetServer::OnCreate()
{
	InitData();
...
void ThailandBillingGetServer::OnDestroy()
{
...
bool ThailandBillingGetServer::SendMessage( CPacket &rkPacket )
{
...
bool ThailandBillingGetServer::CheckNS( CPacket &rkPacket )
{
	return true;             //네트웍 감시 필요없다.
}

int ThailandBillingGetServer::GetConnectType()
{
	return CONNECT_TYPE_THAILAND_BILLING_GET_SERVER;
}

void ThailandBillingGetServer::ProcessTime()
{	
	...
	if( TIMEGETTIME() - m_dwCurrentTime > UPDATE_TIME )       
	{
		...
		{	
			...
			{
				ioHashString sPID;
				pLocal->SendGetCash( "THAS.lsasb025", "127.0.0.1", true , sPID );
			}
		}
...
void ThailandBillingGetServer::DispatchReceive(CPacket& packet, DWORD bytesTransferred)
{
	...
	while( m_recvIO.GetBytesTransferred() > 0 )
	{
		...
		SP2Packet RecvPacket( BTTPK_BILLING_GET_RESULT );
...
void ThailandBillingGetServer::ReceivePacket( CPacket &packet )
{
	g_RecvQueue.InsertQueue( (DWORD)this, packet, PK_QUEUE_SESSION );
}

void ThailandBillingGetServer::PacketParsing( CPacket &packet )
{
	...
	case BTTPK_BILLING_GET_RESULT:
		OnBillingGet( kPacket );
		break;
	default:
...
void ThailandBillingGetServer::OnBillingGet( SP2Packet &rkPacket )
{
...
void ThailandBillingGetServer::SessionClose( BOOL safely/*=TRUE */ )
{
	CPacket packet(ITPK_CLOSE_SESSION);
	ReceivePacket( packet );
}

void ThailandBillingGetServer::OnClose( SP2Packet &rkPacket )
{ 
	if(IsActive())
		OnDestroy();
}