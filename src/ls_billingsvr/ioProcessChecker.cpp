...
FunctionTimeChecker::~FunctionTimeChecker()
{
	if( m_bFrequency )
	{
		...
		{
			if( m_pFileName )
				ProcessLOG.PrintTimeAndLog( 0, "%s %d Line 0x%x : T[%.3f]", m_pFileName, m_iFileLine, m_dwPacketID, fCompleteTime / 1000000 );
		}
	}
}
//////////////////////////////////////////////////////////////////////////
...
void ioProcessChecker::WriteLOG()
{
	...
	sprintf_s( szLOG, "[M:%d(%.3fsec)][MP:%d][L:%d][U:%d(%.3fsec)][G:%d(%.3fsec)][O:%d(%.3fsec)][I:(%.3fsec)][L:%d(%.3fsec)]", m_dwMainLoop, m_fMainTreadMaxTime, m_iMainProcessMaxPacket, m_dwLogDBLoop,
					m_dwUDPLoop, m_fBroadcastTreadMaxTime, m_dwClientAcceptLoop, m_fClientATreadMaxTime, m_dwServerAcceptLoop, m_fServerATreadMaxTime, m_fWorkTreadMaxTime, m_dwMonitoringAcceptLoop, m_fMonitoringATreadMaxTime );
...
void ioProcessChecker::MainServerSendMessage( int iSize )
{
	if( m_dwLogTime == 0 ) return;
...
void ioProcessChecker::LogDBSendMessage( int iSize )
{
	if( m_dwLogTime == 0 ) return;
...
void ioProcessChecker::UDPSendMessage( int iSize )
{
	if( m_dwLogTime == 0 ) return;
...
void ioProcessChecker::BillingRelayServerSendMessage( int iSize )
{
	if( m_dwLogTime == 0 ) return;
...
void ioProcessChecker::MonitoringSendMessage( int iSize )
{
	if( m_dwLogTime == 0 ) return;
