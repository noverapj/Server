...
#include "./ThailandIPBonusOutServer.h"
...
ThailandIPBonusOutServer *ThailandIPBonusOutServer::sg_Instance = NULL;
ThailandIPBonusOutServer::ThailandIPBonusOutServer( SOCKET s, DWORD dwSendBufSize, DWORD dwRecvBufSize ) : CConnectNode( s, dwSendBufSize, dwRecvBufSize )
{
	InitData();
}

ThailandIPBonusOutServer::~ThailandIPBonusOutServer()
{	
}

ThailandIPBonusOutServer &ThailandIPBonusOutServer::GetInstance()
{
	if(sg_Instance == NULL)
	{
		...
		sg_Instance = new ThailandIPBonusOutServer( INVALID_SOCKET, iSendBufferSize, iRecvBufferSize );
	}
	return *sg_Instance;
}

void ThailandIPBonusOutServer::ReleaseInstance()
{		
	SAFEDELETE( sg_Instance );
}

bool ThailandIPBonusOutServer::ConnectTo( bool bStart )
{
	ioINILoader kLoader( "ls_config_billingsvr.ini" );
	kLoader.SetTitle( "NETWORK" );

	char szServerIP[MAX_PATH];
	kLoader.LoadString( "ThailandIPBonusOutServerIP", "", szServerIP, MAX_PATH );

	int iSSPort = kLoader.LoadInt( "ThailandIPBonusOutServerPORT", 9000 );
...
void ThailandIPBonusOutServer::InitData()
{
	m_dwCurrentTime = 0;
	SetActive(false);
}

void ThailandIPBonusOutServer::OnCreate()
{
	InitData();
...
void ThailandIPBonusOutServer::OnDestroy()
{
...
bool ThailandIPBonusOutServer::SendMessage( CPacket &rkPacket )
{
...
bool ThailandIPBonusOutServer::CheckNS( CPacket &rkPacket )
{
	return true;             //네트웍 감시 필요없다.
}

int ThailandIPBonusOutServer::GetConnectType()
{
	return CONNECT_TYPE_THAILAND_IPBONUS_OUT_SERVER;
}

void ThailandIPBonusOutServer::ProcessTime()
{	
	...
	if( TIMEGETTIME() - m_dwCurrentTime > UPDATE_TIME )       
	{
		...
		{	
			...
			{
				ioHashString sPID;
				pLocal->SendIPBonusOut( "THAS.lsasb025", "127.0.0.1", true , sPID );
			}
		}
...
void ThailandIPBonusOutServer::DispatchReceive(CPacket& packet, DWORD bytesTransferred)
{
	...
	while( m_recvIO.GetBytesTransferred() > 0 )
	{
		...
		SP2Packet RecvPacket( BTTPK_IPBONUS_OUT_RESULT );
...
void ThailandIPBonusOutServer::ReceivePacket( CPacket &packet )
{
	g_RecvQueue.InsertQueue( (DWORD)this, packet, PK_QUEUE_SESSION );
}

void ThailandIPBonusOutServer::PacketParsing( CPacket &packet )
{
...
void ThailandIPBonusOutServer::OnIPBonusOut( SP2Packet &rkPacket )
{
	ioHashString sResult;
	rkPacket >> sResult;
	ioLocalParent *pLocal = g_LocalMgr.GetLocal( ioLocalManager::GetLocalType() );
	if( pLocal )
		pLocal->OnRecieveIPBonusOut( sResult );
}

void ThailandIPBonusOutServer::SessionClose( BOOL safely/*=TRUE */ )
{
	CPacket packet(ITPK_CLOSE_SESSION);
	ReceivePacket( packet );
}

void ThailandIPBonusOutServer::OnClose( SP2Packet &rkPacket )
{ 
	if(IsActive())
		OnDestroy();
}