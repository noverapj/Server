...
#include "./ThailandIPBonusServer.h"
...
ThailandIPBonusServer *ThailandIPBonusServer::sg_Instance = NULL;
ThailandIPBonusServer::ThailandIPBonusServer( SOCKET s, DWORD dwSendBufSize, DWORD dwRecvBufSize ) : CConnectNode( s, dwSendBufSize, dwRecvBufSize )
{
	InitData();
}

ThailandIPBonusServer::~ThailandIPBonusServer()
{	
}

ThailandIPBonusServer &ThailandIPBonusServer::GetInstance()
{
	if(sg_Instance == NULL)
	{
		...
		sg_Instance = new ThailandIPBonusServer( INVALID_SOCKET, iSendBufferSize, iRecvBufferSize );
	}
	return *sg_Instance;
}

void ThailandIPBonusServer::ReleaseInstance()
{		
	SAFEDELETE( sg_Instance );
}

bool ThailandIPBonusServer::ConnectTo( bool bStart )
{
	ioINILoader kLoader( "ls_config_billingsvr.ini" );
	kLoader.SetTitle( "NETWORK" );

	char szServerIP[MAX_PATH];
	kLoader.LoadString( "ThailandIPBonusServerIP", "", szServerIP, MAX_PATH );

	int iSSPort = kLoader.LoadInt( "ThailandIPBonusServerPORT", 9000 );
...
void ThailandIPBonusServer::InitData()
{
	m_dwCurrentTime = 0;
	SetActive(false);
}

void ThailandIPBonusServer::OnCreate()
{
	InitData();
...
void ThailandIPBonusServer::OnDestroy()
{
...
bool ThailandIPBonusServer::SendMessage( CPacket &rkPacket )
{
...
bool ThailandIPBonusServer::CheckNS( CPacket &rkPacket )
{
	return true;             //네트웍 감시 필요없다.
}

int ThailandIPBonusServer::GetConnectType()
{
	return CONNECT_TYPE_THAILAND_IPBONUS_SERVER;
}

void ThailandIPBonusServer::ProcessTime()
{	
	...
	if( TIMEGETTIME() - m_dwCurrentTime > UPDATE_TIME )       
	{
		...
		{	
			...
			{
				ioHashString sPID;
				pLocal->SendIPBonus( "THAS.lsasb025", "127.0.0.1", true , sPID );
			}
		}
...
void ThailandIPBonusServer::DispatchReceive(CPacket& packet, DWORD bytesTransferred)
{
	...
	while( m_recvIO.GetBytesTransferred() > 0 )
	{
		...
		SP2Packet RecvPacket( BTTPK_IPBONUS_RESULT );
		...
		if( RecvPacket.IsValidPacket() == true && m_recvIO.GetBytesTransferred() >= dwPacketSize )
		{
			...
			m_recvIO.AfterReceive( dwPacketSize ); //kyg 의심부분 
		}
		else 
			break;
	}
...
void ThailandIPBonusServer::ReceivePacket( CPacket &packet )
{
	g_RecvQueue.InsertQueue( (DWORD)this, packet, PK_QUEUE_SESSION );
}

void ThailandIPBonusServer::PacketParsing( CPacket &packet )
{
...
void ThailandIPBonusServer::OnIPBonus( SP2Packet &rkPacket )
{
	ioHashString sResult;
	rkPacket >> sResult;
	ioLocalParent *pLocal = g_LocalMgr.GetLocal( ioLocalManager::GetLocalType() );
	if( pLocal )
		pLocal->OnRecieveIPBonus( sResult );
}

void ThailandIPBonusServer::SessionClose( BOOL safely/*=TRUE */ )
{
	CPacket packet(ITPK_CLOSE_SESSION);
	ReceivePacket( packet );
}

void ThailandIPBonusServer::OnClose( SP2Packet &rkPacket )
{ 
	if(IsActive())
		OnDestroy();
}