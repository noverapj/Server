...
/*
CString CMD5::GetString(CString str)
{
	MD5_CTX context;
	BYTE digest[16];
	...
	MD5Init (&context);
	MD5Update (&context, (BYTE*)(LPTSTR)(LPCTSTR)str, len);
...
void CMD5::Generate(BYTE* file, UINT length)
{
	ZeroMemory(m_digest, sizeof(m_digest));

	MD5_CTX context;
	
	MD5Init(&context);
	MD5Update(&context, file, length);
	MD5Final(m_digest, &context);
}

char* CMD5::GetDigestHEX()
{
	enum { MAX_DIGEST = 16, };

	ZeroMemory(m_digestHEX, sizeof(m_digestHEX));
	for (int i = 0; i < MAX_DIGEST ; i++)
	{
		char temp[MAX_PATH]="";
		sprintf_s(temp, sizeof(temp), "%02x", m_digest[i]); // BYTE 캐스팅해서 FFFF붙지 않는다.
		strcat_s(m_digestHEX, temp);
	}
	return m_digestHEX;
}
...
void CMD5::MD5Update (MD5_CTX* context, BYTE* input, UINT inputLen)
{
